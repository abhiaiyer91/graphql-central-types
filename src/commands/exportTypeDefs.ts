...
function generateEnumJsFile(graphqlFileContent, filePath) {
  ...
  if (!enumMatches) {
    console.error(
      "File contains 'enum', but does not contain enum definition\n" + filePath
    );
    return;
  }
...
export default class ExportTypeDefs extends Command {
  static description = 'Take all typeDefs and make them exportable';

  static examples = [`$ types exportTypeDefs`];
